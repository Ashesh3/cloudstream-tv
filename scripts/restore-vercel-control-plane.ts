...
function fixtureAdapter(value: unknown): LegacyControlPlaneReader {
  return {
    async readHousehold() { throw new Error("FIXTURE_COLLECTION_READ_FORBIDDEN"); },
    async queryHouseholdCollection(_name: Exclude<LegacyMigrationCollection, "households">, _householdId: string) {
      void _name; void _householdId;
      throw new Error("FIXTURE_COLLECTION_READ_FORBIDDEN");
    },
    async readRecovery() { return structuredClone(value); },
