...
export function SourceWorkbench({ source, roots, devices = [], api, onChanged, onClose }: SourceWorkbenchProps) {
  ...
  const remove = async () => {
    ...
    try { await api.removeRoot(removeRoot.id); invalidateImpactRequest(); setProgramRoots(value => value.filter(root => root.id !== removeRoot.id)); setRemoveRoot(null); setImpact(null); await onChanged(); }
    catch (cause) { setError(safeFailure(cause, "Folder could not be removed.")); }
    ...
    <section ref={workbenchRef} className="source-workbench-shell" role="region" aria-labelledby="source-workbench-title" data-workbench="source-folders" data-material="program-stock" tabIndex={-1} onKeyDown={keyDown}>
      ...
      <div className="source-workbench">
        ...
        <div className="workbench-planes">
          <div className="stage-plane provider-stage-plane" data-workbench-plane="live-provider"><ProviderFolderStage api={api} source={source} selectedProviderNodeIds={new Set(programRoots.flatMap(root => root.providerNodeId ? [root.providerNodeId] : []))} onRootAdded={(root, providerNodeId) => { setProgramRoots(value => [...new Map([...value, { ...root, providerNodeId }].map(item => [item.id, item])).values()]); setNotice(""); void onChanged().catch(() => setNotice(`${root.displayName} was added, but the household ledger could not refresh. The selection remains available.`)); }} onClose={onClose} /></div>
