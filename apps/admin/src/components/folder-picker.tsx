...
export function FolderPicker({ source, roots, api, onChanged, onClose }: {
  ...
  const [restoreRootFocusId, setRestoreRootFocusId] = useState<string | null>(null);
  ...
  const add = async (folder: MediaNodeDto) => { setPending(folder.id); setError(""); try { await api.createRoot(source.id, { providerNodeId: folder.id }); await onChanged(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Root could not be added."); } finally { setPending(null); } };
  const previewRemove = async (root: AssignedRootDto) => { setRestoreRootFocusId(null); setRemoveRoot(root); setImpact(null); setError(""); try { setImpact(await api.rootImpact(root.id)); } catch (cause) { setError(cause instanceof Error ? cause.message : "Impact could not be loaded."); setRestoreRootFocusId(root.id); setRemoveRoot(null); } };
  const cancelRemove = () => { if (removeRoot) setRestoreRootFocusId(removeRoot.id); setRemoveRoot(null); setImpact(null); };
  const confirmRemove = async () => { if (!removeRoot) return; setPending(removeRoot.id); setError(""); try { await api.removeRoot(removeRoot.id); await onChanged(); setRestoreRootFocusId(null); setRemoveRoot(null); setImpact(null); } catch (cause) { setError(cause instanceof Error ? cause.message : "Root could not be removed."); } finally { setPending(null); } };

  if (removeRoot) {
    return <Dialog key="remove-root-confirmation" label="Remove root" onClose={cancelRemove}>
      <header className="dialog-header"><div><p className="eyebrow">Access impact</p><h2>Remove root</h2></div></header>
      <div className="dialog-scroll">
        <p><strong>{removeRoot.displayName}</strong> will no longer be assignable.</p>
        {!impact ? <p>Loading affected devices…</p> : impact.devices.length ? <><p>Affected televisions:</p><ul>{impact.devices.map(device => <li key={device.id}>{device.name}</li>)}</ul></> : <p>No televisions currently use this root.</p>}
        {error && <p className="error-banner" role="alert">{error}</p>}
      </div>
      <footer className="dialog-actions"><button className="button secondary" onClick={cancelRemove}>Cancel</button><button className="button danger" disabled={!impact || pending === removeRoot.id} onClick={() => void confirmRemove()}>Remove root</button></footer>
    </Dialog>;
  }

  return <Dialog key="folder-picker" label="Choose source folders" onClose={onClose} className="folder-sheet">
