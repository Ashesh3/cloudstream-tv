...
export function Sources({ sources, allRoots, api, onRefresh, onAuthorize }: {
  ...
  return <section className="flex flex-col gap-5"><PageHeader eyebrow="Cloud library" title="Sources" description="Connect accounts globally, index their folders, and expose only the roots you choose." action={<div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void onAuthorize("google")}><CloudIcon data-icon="inline-start" />Connect Google Drive</Button><Button onClick={() => void onAuthorize("onedrive")}><CloudIcon data-icon="inline-start" />Connect OneDrive</Button></div>} />
    ...
    {!sources.length ? <Empty title="No cloud sources" body="Connect Google Drive or OneDrive to start indexing folders." icon={<CloudIcon />} /> : <div className="grid gap-4">{sources.map(source => <Card key={source.id}>
      ...
      <CardContent className="grid gap-3 sm:grid-cols-3"><SourceStat label="Last success" value={source.lastSyncCompletedAt ? relativeTime(source.lastSyncCompletedAt) : "Not yet"} /><SourceStat label="Next sync" value={source.nextSyncAt ? relativeTime(source.nextSyncAt) : "Manual"} /><SourceStat label="Folders" value={source.roots.filter(root => root.enabled).length.toString()} /></CardContent>
      {(source.indexProgress || source.lastSyncErrorCode) && <CardContent>{source.indexProgress && <p className="rounded-lg bg-muted/50 p-3 text-sm text-muted-foreground">{progressLabel(source.indexProgress)}</p>}{source.lastSyncErrorCode && <p className="mt-2 rounded-lg bg-destructive/10 p-3 text-sm text-destructive">Last error: {source.lastSyncErrorCode}</p>}</CardContent>}
...
function SourceStat({ label, value }: { label: string; value: string }) { return <div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 truncate text-sm font-medium">{value}</p></div>; }
...
function progressLabel(progress: NonNullable<SourceDto["indexProgress"]>) { return `${progress.mode[0]!.toUpperCase() + progress.mode.slice(1)} · ${progress.processedNodeCount} nodes · ${progress.pendingFolderCount} folders pending`; }
