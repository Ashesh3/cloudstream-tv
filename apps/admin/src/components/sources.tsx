...
export function Sources({ sources, allRoots, devices, api, onRefresh, onAuthorize }: {
  ...
  if (pickerSource) return <section className="source-task-layout"><FolderPicker source={pickerSource} roots={allRoots.filter(root => root.sourceId === pickerSource.id)} devices={devices} api={api} onChanged={onRefresh} onClose={() => setPicker(null)} /></section>;
  return <section className="flex flex-col gap-5"><PageHeader context="Cloud library" title="Sources" description="Connect accounts globally, index their folders, and expose only the roots you choose." action={<div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void onAuthorize("google")}><CloudIcon data-icon="inline-start" />Connect Google Drive</Button><Button onClick={() => void onAuthorize("onedrive")}><CloudIcon data-icon="inline-start" />Connect OneDrive</Button></div>} />
