...
import { AlertTriangleIcon, CheckCircle2Icon, CloudIcon, FolderOpenIcon, LogOutIcon, MonitorIcon, ShieldCheckIcon, TimerIcon } from "lucide-react";
...
export function Settings({ household, snapshot, onSave, onRotate, onLogout }: {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); setPending("defaults"); setError(""); setMessage(""); try { await onSave({ allowNewDeviceRequests: allowed, defaultMediaOrder: order, defaultSlideshowSeconds: Number(seconds) }); setMessage("Household defaults saved."); } catch (cause) { setError(safeFailure(cause, "Settings could not be saved.")); } finally { setPending(""); } };
